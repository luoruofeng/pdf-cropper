...
def load_pdf():
    ...
    if pdf_path:
        # 显示PDF路径，而不是展示PDF内容
        path_label.config(text=f"Selected PDF: {pdf_path}")
...
def main():
    # 初始化 Tkinter 界面
    global root, pdf_path, path_label
    ...
    # 显示PDF路径的Label
    path_label = Label(root, text="Selected PDF: None")
    path_label.grid(row=5, column=0, columnspan=2, padx=5, pady=5)

    Button(root, text="Crop and Save PDF", command=crop_and_save).grid(row=6, column=0, columnspan=2, padx=5, pady=5)
